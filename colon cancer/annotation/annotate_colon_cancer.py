...
# ==============================================================================
# DISEASE 3: Colon Cancer Single-Cell Annotation Script
# ==============================================================================

INPUT_H5AD = r"C:\Users\Steffi Dominic\OneDrive\Desktop\colon cancer\dataset.h5ad"
OUTPUT_DIR = r"C:\Users\Steffi Dominic\.gemini\antigravity\scratch\tcll_cell_annotation\output_colon_cancer"

os.makedirs(OUTPUT_DIR, exist_ok=True)

COLON_MARKERS = {
    'Colon Tumor-Infiltrating T-cell': ['CD3D', 'CD3E', 'CD4', 'CD8A', 'GZMB', 'IFNG'],
    'Colon Epithelial Adenocarcinoma Cell': ['EPCAM', 'KRT8', 'KRT18', 'CDX2', 'MUC2'],
    'Intestinal Macrophage Myeloid': ['CD14', 'FCGR3A', 'CSF1R', 'CD68'],
    'Stromal Fibroblast Cell': ['COL1A1', 'COL1A2', 'ACTA2', 'PDGRA']
}

print(f"[1/5] Loading Colon Cancer Dataset: {INPUT_H5AD}")
...
score_cols = []
for ct, markers in COLON_MARKERS.items():
...
col_map = {f"score_{ct.replace(' ', '_').replace('/', '_').replace('+', 'pos')}": ct for ct in COLON_MARKERS.keys()}
...
try:
    ...
    pd.DataFrame(records).to_csv(os.path.join(OUTPUT_DIR, "colon_cancer_differential_markers.csv"), index=False)
    print("  Differential markers exported to colon_cancer_differential_markers.csv")
except Exception as e:
...
counts_df.to_csv(os.path.join(OUTPUT_DIR, "colon_cancer_cell_distribution.csv"), index=False)

adata.obs.columns = [c.replace('/', '_') for c in adata.obs.columns]

out_h5ad = os.path.join(OUTPUT_DIR, "annotated_colon_cancer.h5ad")
adata.write_h5ad(out_h5ad)

print("\nColon Cancer Cell Annotation Complete!")
